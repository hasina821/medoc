...
const Rows = (movie:any,i:any,admin:any)=>{
    return(
        <tr key={i}>
            <td className={`${Text}`}> 
            ...
            <td className={`${Text} float-right flex items-center gap-2`}>
                {admin ? (
                    <>
                        <button className='border border-border bg-dry  flex items-center gap-2 text-white rounded py-1 px-2'>
    ...
    <div className='overflow-x-scroll overflow-hidden relative w-full'>
        <table className='w-full table-auto border border-border divide-y divide-border text-subMain'>
            ...
            <tbody className='bg-main divide-y divide-gray-800'>
                {data.map((movie:any,i:any)=>Rows(movie,i,admin))}
            </tbody>
        </table>
    </div>
  )
